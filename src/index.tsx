...
const sampleProducts: Record<string, Product[]> = {
  ...
  outdoor: [
    {
      name: "折りたたみチェア 超軽量 アルミ製 耐荷重150kg",
      price: 4980,
  ...
  car: [
    {
      name: "車用掃除機 ハンディクリーナー コードレス 120W",
      price: 4580,
...
// API: カテゴリ別商品取得
app.get('/api/products/:category', (c) => {
...
// API: ランダムに10商品取得
app.get('/api/products/random/10', (c) => {
  ...
  const selected = shuffled.slice(0, Math.min(10, allProducts.length))
  
  const productsWithDescriptions = selected.map(product => ({
    ...product,
    description: generateDescription(product),
    reason: `${product.reviewCount}件以上のレビューで評価${product.rating}を獲得。${product.category}カテゴリで人気急上昇中の注目商品です。`
  }))
    ...
    <body class="bg-gradient-to-br from-pink-50 to-purple-50 min-h-screen">
        <div class="container mx-auto px-4 py-8 max-w-7xl">
            ...
            <div id="loading" class="hidden text-center py-12">
                ...
                <p class="text-gray-600">売れ筋商品をリサーチ中...</p>
            </div>
        </div>
